...
fn test_player_drawn_to_screen() {
    let mut game = set_up_game_with_player();
    let start_pos = game.player_square().unwrap();
...
fn test_pawn_capture_player() {
    let mut game = set_up_game_with_player();
    let player_start_square = game.player_square().unwrap();
    let one_upleft = game.player_square().unwrap() + (UP_I + LEFT_I).cast_unit();
...
fn test_pawn_move_towards_player() {
    let mut game = set_up_game_with_player();
    let two_left = game.player_square().unwrap() + (LEFT_I * 2).cast_unit();
    let one_left = game.player_square().unwrap() + (LEFT_I).cast_unit();
...
fn test_visible_laser() {
    ...
    let inspection_square: WorldSquare =
        game.player_square().unwrap() + game.player_faced_direction();
    game.player_shoot_shotgun();
    game.draw_headless_now();
...
fn test_player_background_is_transparent() {
    let mut game = set_up_game_with_player();
    let inspection_square: WorldSquare = game.player_square().unwrap();
...
fn test_rook_move() {
    ...
    game.place_piece(
        Piece::rook(),
        // three right, one up
        game.player_square().unwrap() + RIGHT_I.cast_unit() * 3 + UP_I.cast_unit() * 1,
    )
    .expect("place rook");

    let one_up = game.player_square().unwrap() + UP_I.cast_unit();
...
fn test_rook_capture() {
    ...
    game.place_piece(
        Piece::rook(),
        game.player_square().unwrap() + RIGHT_I.cast_unit() * 3,
    )
    .expect("place rook");
...
fn test_king_move() {
    ...
    game.place_piece(
        Piece::king(),
        // three right, one up
        game.player_square().unwrap() + diag_up_right * 2,
    )
    .expect("place king");

    let king_end_square = game.player_square().unwrap() + diag_up_right;
...
fn test_knight_move() {
    ...
    game.place_piece(
        Piece::knight(),
        // three right, one up
        game.player_square().unwrap() + RIGHT_I.cast_unit() * 3 + UP_I.cast_unit(),
    )
    .expect("place knight");

    let end_square = game.player_square().unwrap() + RIGHT_I.cast_unit();
...
fn test_no_move_into_check() {
    let mut game = set_up_game_with_player();
    let rook_square = game.player_square().unwrap() + LEFT_I.cast_unit() + UP_I.cast_unit() * 3;
...
fn test_draw_danger_squares() {
    ...
    game.place_piece(Piece::rook(), rook_square)
        .expect("place rook");
    let danger_square = game.player_square().unwrap() + LEFT_I.cast_unit();
    game.draw_headless_now();
...
fn test_no_step_on_block() {
    let mut game = set_up_game_with_player();
    let block_square = game.player_square().unwrap() + RIGHT_I.cast_unit();
...
fn test_rook_stopped_by_block() {
    let mut game = set_up_game_with_player();
    let rook_start_square = game.player_square().unwrap() + RIGHT_I.cast_unit() * 4;
    let rook_end_square = game.player_square().unwrap() + RIGHT_I.cast_unit() * 2;
    let block_square = game.player_square().unwrap() + RIGHT_I.cast_unit();
    game.place_block(block_square);
...
fn test_some_indicator_that_a_pawn_might_step_out_of_the_path_of_a_rook_immediately_before_that_rook_moves(
    ...
    let mut game = set_up_game_with_player();
    let pawn_square = game.player_square().unwrap() + RIGHT_I.cast_unit() * 3;
    let rook_square = game.player_square().unwrap() + RIGHT_I.cast_unit() * 4;
    let square_to_check = game.player_square().unwrap() + RIGHT_I.cast_unit() * 1;
