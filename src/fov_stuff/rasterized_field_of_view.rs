...
struct TopDownPortalTarget {
    ...
    portal_depth: u32,
    portal_rotation_to_target: NormalizedOrthoAngle,
}
...
pub trait RasterizedFieldOfViewFunctions {
    ...
    fn try_add_visible_relative_square_with_depth_but_no_rotation(
        &mut self,
        relative_square: impl Into<WorldStep>,
        visibility: &TopDownPortalShape,
        portal_depth: u32,
...
type TopDownPortalShape = DefaultSquareVisibilityType;
...
// TODO: this made more sense before rasterized fields of view had a view root.  View root could be the square that has the portals?  (no that's a cop out)
...
impl SquareOfTopDownPortals {
    ...
    fn lone_portal_rotation_or_panic(&self) -> NormalizedOrthoAngle {
    ...
    fn one_portal_deeper(&self, forward_rotation_through_portal: NormalizedOrthoAngle) -> Self {
        todo!()
    ...
    fn relative_position(&self) -> WorldStep {
        ...
        self.0
            .map_of_top_down_portal_exit_shapes_by_coordinates
            .keys()
            .next()
            .unwrap()
            .0
...
impl RasterizedFieldOfViewFunctions for RasterizedFieldOfView {
    fn visibility_map_to_top_down_portals(
        ...
        vis_map
            .iter()
            .map(|(&rel_square, visibility)| {
                TopDownPortal::new_with_exit_shape(
                    rel_square,
                    TopDownPortalTarget::new(
                        root.other_square_relative_to_absolute(rel_square),
                        0,
                        0,
                    ),
                    visibility.to_owned(),
    ...
    fn try_add_visible_local_relative_square(
        ...
        self.try_add_visible_relative_square_with_depth_but_no_rotation(
            relative_square,
            visibility,
            0,
        )
    ...
    fn try_add_visible_relative_square_with_depth_but_no_rotation(
        ...
        // TODO: do we need to consider non-overlapping visibilities in the same square?
        if self.can_see_relative_square_at_depth(relative_square, portal_depth) {
            return Err(());
        }

        let new_top_down_portal = TopDownPortal::new_with_exit_shape(
            relative_square,
            TopDownPortalTarget::new(
        ...
        self.add_top_down_portal(new_top_down_portal);
        Ok(())
    }
    fn add_visible_local_relative_square(
        &mut self,
    ...
    fn visible_relative_squares(&self) -> StepSet {
        self.map_of_top_down_portal_exit_shapes_by_coordinates
            .iter()
    ...
    fn absolute_top_down_portal_entrance_shapes_at_relative_square(
        ...
        self.top_down_portals_for_relative_square(relative_square)
            .iter()
            .map(|x| x.absolute_entrance_shape())
            .collect()
    }
    ...
    fn absorb(&self, other: &Self) -> Self {
        // This check is skipped because everything but the view root is relative to the view root.
        // The other view root is discarded, and the other relative top-down portals are absorbed.
        // assert_eq!(self.view_root, other.view_root);

        // need to account for portal rotations if view roots have different rotations
        // assert_eq!(self.view_root.direction(), other.view_root.direction());

        Self::from_top_down_portals(
            self.view_root,
            self.top_down_portals_vec()
                .into_iter()
                .chain(other.top_down_portals_vec()),
        )
    }
...
// These are only the private functions, put the public ones in the trait for easier visibility (bad idea?)
impl RasterizedFieldOfView {
    ...
    fn add_top_down_portal(&mut self, portal: TopDownPortal) {
        ...
        let new_portal_shape: TopDownPortalShape = portal.shape_in_exit_frame.clone();
    ...
    fn can_see_local_relative_square(&self, rel_square: WorldStep) -> bool {
        self.can_see_relative_square_at_depth(rel_square, 0)
    }
    fn can_see_relative_square_at_depth(&self, rel_square: WorldStep, portal_depth: u32) -> bool {
        self.map_of_top_down_portal_exit_shapes_by_coordinates
            .keys()
            .any(|positioned_vis| {
                positioned_vis.1.portal_depth() == portal_depth && positioned_vis.0 == rel_square
            })
    }
    ...
    pub fn from_top_down_portals(
        ...
        assert!(
            ...
            "Failed to create rasterized field of view because of conflicting portal entrances: {:#?}\nFOV edge points on unit square: {:?}",
            top_down_portals_with_conflicting_entrances,
            top_down_portals_with_conflicting_entrances.iter().map(
                |visibility_pair|{
                    visibility_pair.map(|square_vis|{
                        square_vis.absolute_entrance_shape().where_border_touches_unit_square()
                    })
                }
            ).collect_vec()
        );
        Self {
            view_root,
            map_of_top_down_portal_exit_shapes_by_coordinates: iter
                .into_iter()
                .map(|x| ((x.relative_position, x.target), x.shape_in_exit_frame))
                .collect(),
        }
    }
    ...
    fn filtered(
        ...
        let filtered_portal_map = self
            ...
            .collect();
        Self {
            map_of_top_down_portal_exit_shapes_by_coordinates: filtered_portal_map,
            ..self.clone()
        }
    }
    ...
    fn top_down_portals_vec(&self) -> Vec<TopDownPortal> {
        self.top_down_portals().collect()
    }

    pub fn relative_top_down_portal_entrance_shapes_at_relative_square(
        &self,
        rel_square: impl Into<WorldStep>,
    ) -> Vec<TopDownPortalShape> {
        let view_root_rotation = self.view_root_orientation_relative_to_default();

        self.absolute_top_down_portal_entrance_shapes_at_relative_square(rel_square)
            .into_iter()
            .map(|entrance_shape| entrance_shape.quarter_rotated_ccw(-view_root_rotation))
            .collect_vec()
    }
}

impl TopDownPortal {
    fn new_with_exit_shape(
    ...
    fn new_with_entrance_shape(
        rel_pos: impl Into<WorldStep>,
        target: TopDownPortalTarget,
        entrance_shape: TopDownPortalShape,
    ) -> Self {
        Self::new_with_exit_shape(
            rel_pos,
            target,
            entrance_shape.quarter_rotated_ccw(target.portal_rotation_to_target),
        )
    }
    ...
    pub fn with_portal_depth(mut self, depth: u32) -> Self {
        self.target.portal_depth = depth;
        self
    }
    pub fn with_forward_portal_rotation(
        mut self,
        forward_portal_rotation: impl Into<NormalizedOrthoAngle>,
    ) -> Self {
    ...
    pub fn absolute_entrance_shape(&self) -> TopDownPortalShape {
        self.shape_in_exit_frame
            .quarter_rotated_ccw(-self.target.portal_rotation_to_target)
    }
    pub fn absolute_exit_shape(&self) -> TopDownPortalShape {
        self.shape_in_exit_frame
    }
    pub fn portal_depth(&self) -> u32 {
        self.target.portal_depth
    }
    pub fn portal_rotation_to_target(&self) -> NormalizedOrthoAngle {
    ...
    pub fn is_about_equal_to(&self, other: Self) -> bool {
...
impl TopDownPortalTarget {
    ...
    fn new(
        absolute_square: impl Into<WorldSquare>,
        portal_depth: u32,
        portal_rotation_to_target: impl Into<NormalizedOrthoAngle>,
    ) -> Self {
        Self {
...
mod tests {
    ...
    fn test_rounding_towards_full_visibility() {
        ...
        rasterized_fov.add_partially_visible_local_relative_square(
            STEP_UP,
            &TopDownPortalShape::new_partially_visible_from_visible_half_plane(
                HalfPlaneCuttingLocalSquare::top_half_plane().extended(0.5 - 1e-2),
            ),
        );
        rasterized_fov.add_partially_visible_local_relative_square(
            STEP_DOWN,
            &TopDownPortalShape::new_partially_visible_from_visible_half_plane(
                HalfPlaneCuttingLocalSquare::top_half_plane().extended(0.5 - 1e-4),
            ),
        );
    ...
    fn test_add_a_view_of_a_square() {
        ...
        rasterized_fov.add_top_down_portal(TopDownPortal {
            ...
            target: TopDownPortalTarget {
                ...
                portal_depth: 1,
                portal_rotation_to_target: NormalizedOrthoAngle::default(),
            },
    ...
    fn test_no_overlapping_top_down_portal_entrances__two_fully_visible() {
        let portal1 = TopDownPortal::new_with_exit_shape(
            STEP_UP,
            TopDownPortalTarget {
        ...
        let portal2 = TopDownPortal::new_with_exit_shape(
            STEP_UP,
            TopDownPortalTarget {
    ...
    #[test]
    #[should_panic]
    fn test_top_down_portals_in_one_square_are_in_the_same_square() {
        let portal1 = TopDownPortal::new_with_exit_shape(
            STEP_UP,
            TopDownPortalTarget {
                absolute_square: point2(5, 5),
    ...
    fn test_top_down_portal_one_portal_deeper() {
        let top_down_portal = TopDownPortal::new_with_exit_shape(
            ...
            SquareVisibility::new_partially_visible_from_visible_half_plane(
                HalfPlaneCuttingLocalSquare::new_with_inside_down(0.0),
            ),
        );
    ...
    fn test_rasterized_field_of_view_with_one_square_seen_through_portal__identity_case() {
        ...
        assert_eq!(new_rasterized_fov.top_down_portals_vec().len(), 1);

        let deeper_top_down_portal = new_rasterized_fov.top_down_portals_vec()[0];
    ...
    fn test_rasterized_field_of_view_with_one_square_seen_through_portal__portal_moved_and_rotated()
    {
        ...
        let absolute_deeper_top_down_portal = new_rasterized_fov
            ...
            .top_down_portals_vec()[0];
        assert_eq!(
            absolute_deeper_top_down_portal.relative_position,
            (0, 8).into()
        );

        let relative_deeper_top_down_portal = new_rasterized_fov.top_down_portals_vec()[0];
        assert_eq!(
    ...
    fn test_local_positions_are_relative_to_view_root_orientation() {
        ...
        assert_eq!(
            rfov.top_down_portals_vec().first().unwrap().target_square(),
            (6, 2).into()
        );
    }
    #[test]
    ...
    #[test]
    fn test_top_down_portal_entrances_on_one_square_overlap_check__exit_rotation_should_not_matter()
    {
        let rel_pos = (2, 2);
        (0..4)
            .cartesian_product(0..4)
            .for_each(|(portal_turns_1, portal_turns_2)| {
                let p1 = TopDownPortal::new_with_entrance_shape(
                    rel_pos,
                    TopDownPortalTarget::new((5, 0), 1, portal_turns_1),
                    DefaultSquareVisibilityType::new_orthogonal_half_visible(UP),
                );
                let p2 = TopDownPortal::new_with_entrance_shape(
                    rel_pos,
                    TopDownPortalTarget::new((0, 5), 1, portal_turns_2),
                    DefaultSquareVisibilityType::new_orthogonal_half_visible(DOWN),
                );
    ...
    fn test_top_down_portal_constructors_are_consistent() {
        ...
        let top_down_portal_from_entrance =
            TopDownPortal::new_with_entrance_shape(rel_square, target, entrance_shape);

        assert!(top_down_portal_from_exit.is_about_equal_to(top_down_portal_from_entrance));
    }
    #[test]
    fn test_rfov_from_top_down_portals__consistency() {
        let portal_entrance_rel_pos: WorldStep = (4, 5).into(); // arbitrary
        let portal_exit_square: WorldSquare = (111, 111).into(); // arbitrary
        let portal_depth = 4; // arbitrary
        let view_root_square: WorldSquare = (30, 30).into(); // arbitrary

        #[derive(Debug)]
        struct TestParams {
            root_rotation: NormalizedOrthoAngle,
            forward_portal_rotation: NormalizedOrthoAngle,
            portal_exit_direction: NormalizedOrthoAngle,
            expected_absolute_entrance_orientation: NormalizedOrthoAngle,
            expected_relative_entrance_orientation: NormalizedOrthoAngle,
        }
        impl TestParams {
            // TODO: This kind of constructor should be derivable.  No way someone hasn't made a macro for it already.
            pub fn new(
                root_rotation: impl Into<NormalizedOrthoAngle>,
                forward_portal_rotation: impl Into<NormalizedOrthoAngle>,
                portal_exit_direction: impl Into<NormalizedOrthoAngle>,
                expected_absolute_entrance_orientation: impl Into<NormalizedOrthoAngle>,
                expected_relative_entrance_orientation: impl Into<NormalizedOrthoAngle>,
            ) -> Self {
                Self {
                    root_rotation: root_rotation.into(),
                    forward_portal_rotation: forward_portal_rotation.into(),
                    portal_exit_direction: portal_exit_direction.into(),
                    expected_absolute_entrance_orientation: expected_absolute_entrance_orientation
                        .into(),
                    expected_relative_entrance_orientation: expected_relative_entrance_orientation
                        .into(),
                }
            }
        }
        ...
        (0..4)
            ...
            .cartesian_product(0..4)
            .map(
                |((root_rotation, forward_portal_rotation), portal_exit_direction)| {
                    let expected_absolute_entrance_orientation =
                        portal_exit_direction - forward_portal_rotation;
                    TestParams::new(
                        root_rotation,
                        forward_portal_rotation,
                        portal_exit_direction,
                        expected_absolute_entrance_orientation,
                        expected_absolute_entrance_orientation - root_rotation,
                    )
                },
            )
            .into_iter()
            .for_each(|test_params| {
                let root_rotation = test_params.root_rotation;
                let forward_portal_rotation = test_params.forward_portal_rotation;
                let portal_exit_direction = test_params.portal_exit_direction;

                let absolute_exit_shape = portal_direction_to_shape(portal_exit_direction);

                let top_down_portal = TopDownPortal::new_with_exit_shape(
                    portal_entrance_rel_pos,
                    TopDownPortalTarget::new(
                        portal_exit_square,
                        portal_depth,
                        forward_portal_rotation,
                ...
                let retrieved_portal = rfov.lone_top_down_portal_or_panic();
                let retrieved_absolute_entrance_shape = retrieved_portal.absolute_entrance_shape();

                let retrieved_absolute_exit_shape = retrieved_portal.absolute_exit_shape();

                let expected_absolute_entrance_shape =
                    portal_direction_to_shape(test_params.expected_absolute_entrance_orientation);

                let expected_relative_entrance_shape =
                    portal_direction_to_shape(test_params.expected_relative_entrance_orientation);

                let retrieved_relative_entrance_shape = rfov
                    .relative_top_down_portal_entrance_shapes_at_relative_square(
                        portal_entrance_rel_pos,
                    )[0];
